...
> 個別の整合規約・lint は各 skill 自身の `scripts/check-*.ts`（決定論はコードへ）が持ち、
> 本書は**判断**だけを書く。
...
- **欠落部品（前方参照・未実装）**: 流れを機械的に畳むには spec/AC 間の **`supersedes` エッジ**（どの過去 AC を
  ...
  `apps/agentops/src/` 側の決定論ロジックとして実装する
  （[ARCHITECTURE](../ARCHITECTURE.md) の store を SoT とする方針に乗せる）。
...
本書は判断のみ。次は将来 `apps/agentops/scripts/check-*.ts` / CI 側で機械強制する候補：
