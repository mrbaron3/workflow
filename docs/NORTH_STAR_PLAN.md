...
| B4 | **✅ 構造実装済み／grounded不一致観測待ち** — 全required Perspective承認後にrequired check／外部blocking reviewが棄却したrevisionをsurrogate/oracle mismatchとして導出。同一revision一票・次reviewerへ詳細なし件数だけを返し、edge/adversarial/falsifiable検査を強化する（ADR-0011）。厳密なhidden ground truthや永続surrogate test suiteではないため「正解不要」を過大主張しない | 正解全文が無い開放タスクでも独立反証から検証器を改善する | PR-native運用 | 実PRでmismatch→次revision promptへのopaque signal→同クラス再発防止を1件grounded観測 |
