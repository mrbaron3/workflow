# ADR-0018: 代理検証器の承認と独立 oracle の棄却を、不透明な検証信号として次 revision へ返す

- 状態: 採択・吸収済み（2026-07-23 決定、2026-07-28 現行構造へ移植）
- 由来: 保存ブランチ `feat/surrogate-verifier-oracle-calibration` の `df9d1c9`
- 参考:
...
保存時の文書番号は ADR-0011 だったが、現行 `main` では同番号が標準 OCI image の決定に
割り当て済みである。append-only な決定記録を上書きせず、判断の意味を変えずに ADR-0018 として移植する。
...
内部 Perspective が既に `request_changes` を出した revision も、代理検証器が失敗を捕捉済みなので
mismatch に数えない。
...
- − 論文は hidden deterministic verifier を前提にする。本ハーネスの外部信号は同等の ground truth ではなく、
  転用可能な設計要素だけを採用したもの。
