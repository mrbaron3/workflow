...
- **DATA-execution-012 `PR.origin`** — `issue-pipeline|repository-discovery`のadditive discriminator。既存rowはdefault `issue-pipeline`で読み、repository pollが外部PR番号をdedup keyとして作ったrowだけ`repository-discovery`を持つ。後者のsynthetic Issueはreview状態を既存state machineへ写すためのwork unitで、operatorが個別登録する設定実体ではない。attacker-controlled headを通常Generatorへ渡さないため`assignedAgent=null`を保持する。
