...
- **DATA-execution-011 `RevisionGateSnapshot`** — current revisionの必須Perspective verdict、deterministic gates、GitHub check state、unresolved blocking threadのid/body/path/line、mergeabilityを1回の判定証拠として保持する。mergeは同じhead SHAの最新snapshotだけを消費する。`IntakeRecord.sourceClosedAt/sourceCloseError`は分割Source Issue集約closeの成功または再試行待ちを表す。
- **DATA-execution-012 `PR.origin`** — `issue-pipeline|repository-discovery`のadditive discriminator。既存rowはdefault `issue-pipeline`で読み、repository pollが外部PR番号をdedup keyとして作ったrowだけ`repository-discovery`を持つ。後者のsynthetic Issueはreview/repair状態を既存state machineへ写すためのwork unitで、operatorが個別登録する設定実体ではない。
