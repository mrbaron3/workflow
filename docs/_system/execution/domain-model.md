...
- **DOM-execution-007 legacy/store 審査ゲートの状態遷移** — `gate.backend=store` では `build-approved`（`LANG-evaluation-016`）→ `needs-human-review` → 人間承認 → `released`。GitHub PR-native 経路では `DOM-execution-013/014` が優先し、同一 current-head の全機械ゲート通過後に自動 merge / `released`、自動処理不能時だけ `needs-human-review` へ進む。
...
- **DOM-execution-010 reviewer workspace integrity の不変条件** — Reviewer は評価対象の generator worktree を cwd にせず、Perspective 専用 Reviewer Workspace（`LANG-execution-015`）だけを読む・test する。findings は checkout 外の Review Evidence Sidecar（`LANG-execution-016`）へ書く。reviewer が source/config を変更した場合はその Perspective の findings を採用しない。一方、明示分類された Environment Artifact Mutation（`LANG-execution-017`）だけなら変更を帰属・記録したうえで findings を採用し、disposable checkout と共に破棄する。いずれの場合も build commit と generator worktree は不変である。
- **DOM-execution-011 verification fail-closed不変条件** — 自動採点methodを宣言したACは、そのmethod固有のintrinsic checkまたは設定commandが実行された場合だけsatisfiedになれる。未設定・起動失敗・非zero exitを別methodやsuite全体greenで補完しない。
- **DOM-execution-012 criterion isolation不変条件** — 非unit commandはACごとに起動し、`AGENTOPS_AC_ID`・`AGENTOPS_ISSUE_ID`・method・expectedを環境境界で渡す。1つのACの成功を同methodの兄弟ACへ複製しない。
- **DOM-execution-013 revision approval不変条件** — approveはPR全体やbranch名でなく`LANG-execution-020`のcurrent head SHAにだけ属する。headが変われば旧EvalRun/check/reviewは監査履歴として残るがmerge資格へ寄与しない。同じheadに`LANG-execution-022`が1件でもあればapproveを導出しない（ADR-0009）。
- **DOM-execution-014 autonomous delivery不変条件** — 通常経路はPR作成→current revision評価→必要なら同branch修正push→新revision再評価→atomic merge→released→依存解除である。人間mergeを通常のprogress条件にせず、自動処理不能だけを`needs-human-review`へ昇格する（ADR-0009）。
- **DOM-execution-015 repository registration scope不変条件** — 自動管理の登録単位はrepositoryであり、PR/Issue単位の登録を作らない。各pollは対象baseのOpen PRを外部番号で冪等化し、`LANG-execution-023`の未評価current headだけをreviewする。Draftはreview対象だがmerge不可、fork headはwrite authorityを推測せず対象外とする。
- **DOM-execution-016 external mutation identity不変条件** — GitHub Source Issue由来のbranch・worktree・session・PRは`LANG-execution-024`だけから同じsample identityを導き、Store-local Issue IDをexternal mutation keyにしない。push前にcanonical repositoryとremote branch、およびopen PRのexternal Issue/release/body marker/qualified closing targetの意味的一致をread-onlyで検証する。既存branchにexact-correlated open PRがない、または複数候補がある場合はpush/PR mutation前にfail-closedとする。
