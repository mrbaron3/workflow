...
- **DOM-registration-control-003 Supervised Component** — Issue monitor、PR monitor、およびcompatibility topology
  だけのforwarderの `(registration_id, component, registration_version)` identity。desired false または DB 不達で停止する。
- **DOM-registration-control-004 Convergent Work Item** — repository、entity kind
  (`issue|pull-request|repository-head`)、number／identity、GitHub `updated_at` で表すlogical work。
  GitHub event名はSource Eventとしてentity kindと分離する。Webhook delivery keyとpoll cursorは異なるtransport
  identityだが、legacy kindから導出済みのkeyも含めlogical idempotency identityは共有する。
...
- **DOM-registration-control-007 Topology-aware component status** — Issue Monitor、PR Monitor、Execution、Queueと、
  compatibility topologyだけのForwarderを別componentとして扱い、それぞれdesired、actual、observedAt、freshness、stale reason、
