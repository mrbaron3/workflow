...
- **ARCH-registration-control-004 Retry idempotency** — Registration create と delivery retry は
  ...
  stale observed-attempt、processing/processed delivery を 409 で拒否する。
...
- **ARCH-registration-control-007 Experience design gate** — Control API 起動前に
  ...
  unapproved、digest mismatch、mixed revision、incomplete capability、coverage 欠落は fail closed にする。
