...
- **ARCH-webhook-004 Local Forwarder Manager** — enabled registrationごとに`gh webhook forward --repo --events --url --secret`を監督し、`/hook` verifierと同じsecretを渡す。設定変更時に対象processだけを再構成する。coreはprocess形を知らない。
