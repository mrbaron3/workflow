...
> このコンテキストは**永続実体を持たない**。構造化正本は
> `apps/agentops/src/runtime/schema.ts` の runtime-neutral な Zod 契約。
...
- **DATA-container-runtime-002 runtime-neutral 契約** — cross-boundary 語彙は
  `apps/agentops/src/runtime/schema.ts` の Zod schema:
...
- **DATA-container-runtime-005 grounded evidence artifact** —
  `deploy/scripts/runtime-smoke.ts` が出力する JSON 証跡
