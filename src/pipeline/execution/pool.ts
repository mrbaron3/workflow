 ...
 * (ADR-0006 E4 panel.maxConcurrent — a saturation guard for machine / rate limits). Results keep
 * input order regardless of completion order. A worker that throws rejects the whole call (the
 * caller decides how to surface it); it does not silently drop an item.
 */
