...
export function isSurrogateOracleMismatch(
  ...
  const independentOracleRejected =
    snapshot.checks.some((check) => check.status === 'failure')
    || snapshot.unresolvedBlockingThreadIds.length > 0;
  return independentOracleRejected;
}
...
export function surrogateOracleMismatchRevisions(
  ...
  for (const snapshot of store.db.revisionGateSnapshots) {
    if (prId !== undefined && snapshot.prId !== prId) continue;
    if (isSurrogateOracleMismatch(snapshot)) {
      revisions.add(snapshot.revisionId);
    }
  }
  return [...revisions];
}
