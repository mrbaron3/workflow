 ...
 * so the sandbox needs no npm install. After scaffolding:
 * npx tsx apps/agentops/scripts/real-panel-run.ts
 */
...
// feeds the findings back into the next generator session. e.g.
// MAX_REPAIRS=1 npx tsx apps/agentops/scripts/real-run-sandbox.ts
...
console.log(`\nNext:  LENSES=codeQuality npx tsx apps/agentops/scripts/real-panel-run.ts   (cheap: generator + 1 review)`);
if (HARD) console.log(`       (HARD run: attempt 1 may fail AC-3 → watch the repair loop drive attempt 2)`);
if (!genModel) console.log(`       To reliably trip the repair loop, weaken the coder:  GEN_MODEL=haiku HARD=1 MAX_REPAIRS=1 npx tsx apps/agentops/scripts/real-run-sandbox.ts`);
