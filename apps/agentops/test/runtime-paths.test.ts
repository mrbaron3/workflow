...
describe('host-path dependency scanner', () => {
  ...
  it('AC-CISO-011 the real repository build/runtime surface carries no macOS absolute path', () => {
    // Regression guard for issue #11: app source/config and the OCI composition
    // contract are separate roots, and both must remain host-neutral.
    expect(scanForHostPathDependencies(AGENTOPS_PACKAGE_ROOT)).toEqual([]);
    expect(scanForHostPathDependencies(REPOSITORY_ROOT, [
      path.join(REPOSITORY_ROOT, 'deploy', 'Containerfile'),
    ])).toEqual([]);
  });
});
