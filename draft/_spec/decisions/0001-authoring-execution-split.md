...
> ⚠️ **ADR-0008 で更新**: 本 ADR の「Epic = 1 spec.md」（O1/D4）と「署名記録の最初の永続先 = epic 状態
> オブジェクト」（O2・§4）は、**spec を粒度非依存の著述・署名単位**とし **epic を下流のロードマップ
> grouping** へ降格した ADR-0008（D50-D54）で更新。署名の永続先は **spec 状態オブジェクト**。原文は決定記録
> として保持する。
...
| D19 | オーサリング層（M20）は **skill 駆動の人間ワークフロー**として実装（既存 `draft-spec` skill を本合意の出力形に拡張/置換）（**ADR-0005 で更新: AI 補助 + コード強制・特定 skill 非依存**） | サービスではなく協業を駆動する skill |
