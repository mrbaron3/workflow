# 仕様ワークスペース (draft/_spec)

本ディレクトリは、AI組織運用ハーネスの **モジュール別 詳細仕様** の **下書き（draft）** である。
...
| M21 | Design Planner（新規） | approved spec.md → **三層設計**（system 層=ドメイン/データ/アーキ＝global 単一 SoT・追加のみ / epic=design-delta / slice=コンポーネント=PR サイズ）+ issue 分解（β）。**実装はカデンツで 2 skill**（`to-system-design`=基本/DB設計・境界コンテキスト単位 / `to-slice-design`=詳細設計・epic 単位・D49）・図表は派生・**層別カデンツ**。AI 著者・人間 override 任意（ADR-0004 D30-D34 / **ADR-0007 D41-D49**） | ADR-0001, ADR-0002, ADR-0003, ADR-0004, ADR-0007, §11, §12 | `.claude/skills/to-system-design/`, `.claude/skills/to-slice-design/`, `src/design/lint.ts`（新規）, `agents/issue-planner.md`（分割元） | 下書き（skill 雛形あり） |
...
モジュールの線形順ではなく、[北極星](../../docs/NORTH_STAR.md)の三能力（**自律 × 評価 × 改善**）を閉じる
...
- **自律**: M20 spec.md → M21 設計/分解 → **M22 設計審査（薄）** → M05 resolve → M03 Coordinator（薄）→ M06 Generator → PR。
- **評価**: M07 Evaluator + M09 Harness（薄・実 grader 1〜2種）→ scorecard / evidence。
  **M22 Design Reviewer（薄）**: spawn 前に設計を独立審査（決定的 tier=被覆/排他/参照 + 軽い整合性チェック）→ DesignScorecard。
  設計の整合性 grader の作り込み（5軸 rubric）は loop 2 へ。
- **改善**: M08 Repair（薄・生成↔評価を閉じる）/ M10 Eval Curator（薄・**loop 1 の失敗を回帰 eval に昇格**。
  設計起因失敗 `design_failure` の携帯も含む）。
...
> 「loop 1 では決定的 tier のみ・整合性審査ごと loop 2」へ更に薄める選択も可。順番・粒度の変更があれば指定する。
