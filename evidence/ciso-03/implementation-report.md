...
| Forwarder supervision | pinned `gh-webhook@v0.2.0`を含むfixed `gh webhook forward --repo --events` adapter、secretを除くallowlist環境、repository identity検証、exit/backoff/restart、actual state |
| persist-before-ack/dedup/retry | HMAC検証、PostgreSQL jsonb同値によるglobal delivery dedup、DB clock retry、commit後202、lease/expiry/backoff、accepted/rejected retry attempt audit |
| webhook + poll convergence | repository/kind/number/updated-atのlogical idempotency key、同一key advisory serialization、DB unique/single-flight制約、busyはmonitor failureにせずcursorを保持 |
| desired vs actual | version/freshness付き`monitor_actual_states`、最新job failureとdelivery identity/reason/attemptsを含むrepeatable-read anomaly-first projection |
...
| Go race | PASS (`go test -race ./...`、25 test functions) |
| Go vet | PASS (`go vet ./...`) |
| TypeScript contract targeted | PASS (5 tests) |
| TypeScript PostgreSQL integration | PASS (16/16、internal-only Apple Container network) |
