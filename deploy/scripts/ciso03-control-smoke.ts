...
async function main(): Promise<void> {
  ...
  run(['build', '--target', 'control', '-t', controlImage, '-f', repositoryPath('deploy', 'Containerfile'), '.']);
  run(['build', '--target', 'control-test', '-t', controlTestImage, '-f', repositoryPath('deploy', 'Containerfile'), '.']);
  const forbiddenGh = spawnSync(
    'container',
    ['run', '--rm', '--entrypoint', 'gh', controlImage, 'webhook', '--help'],
    { cwd: root, stdio: 'ignore' },
  );
  if (forbiddenGh.status === 0) {
    throw new Error('credential-free control image unexpectedly contains the gh CLI');
  }
