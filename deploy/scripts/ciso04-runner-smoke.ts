...
async function main(): Promise<void> {
  ...
  run(['build', '--target', 'runtime', '-t', runtimeImage, '-f', repositoryPath('deploy', 'Containerfile'), '.']);
  run(['build', '--target', 'runner', '-t', runnerImage, '-f', repositoryPath('deploy', 'Containerfile'), '.']);
  ...
  run([
    ...
    '--configLoader',
    'runner',
    'apps/agentops/test/control-store.integration.test.ts',
  ]);
  pass('postgresRunnerIntegration', {
    suite: 'apps/agentops/test/control-store.integration.test.ts',
    network,
    publishedPorts: 0,
  });
  postgresExec(
  ...
  const heartbeatEvidence = JSON.parse(run([
    ...
    runnerImage,
    'apps/agentops/dist/scripts/ciso04-heartbeat-block.js',
